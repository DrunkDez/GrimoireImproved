...
// ── DotSelector — unchanged logic, unchanged props ───────────────────────────
...
export default function EditCharacterPage() {
  ...
  const fetchCharacter = async () => {
    if (!characterId) { console.error("No character ID available"); return }
    try {
  ...
  const updateCharacter  = (field: string, value: any) => { if (!character) return; setCharacter({ ...character, [field]: value }) }
  const updateAttribute  = (attr: string, value: number) => { if (!character) return; setCharacter({ ...character, attributes: { ...character.attributes, [attr]: value } }) }
  const updateAbility    = (ability: string, value: number) => { if (!character) return; setCharacter({ ...character, abilities: { ...character.abilities, [ability]: value } }) }
  const updateSphere     = (sphere: string, value: number) => { if (!character) return; setCharacter({ ...character, spheres: { ...character.spheres, [sphere]: value } }) }
  const updateBackground = (bg: string, value: number) => { if (!character) return; setCharacter({ ...character, backgrounds: { ...character.backgrounds, [bg]: value } }) }

  const addMerit = (merit: any) => {
    if (!character) return
    const currentMerits = character.merits || []
    if (currentMerits.some(m => m.id === merit.id)) {
      toast({ title: "Already Added", description: "This merit is already selected", variant: "destructive" }); return
    }
    setCharacter({ ...character, merits: [...currentMerits, { id: merit.id, name: merit.name, cost: Math.abs(merit.cost) }] })
  ...
  const addFlaw = (flaw: any) => {
    ...
    if (currentFlaws.some(f => f.id === flaw.id)) {
      toast({ title: "Already Added", description: "This flaw is already selected", variant: "destructive" }); return
    }
    if (currentTotal + flawCost > 7) {
      toast({ title: "Maximum Flaws", description: "You cannot take more than 7 points in flaws", variant: "destructive" }); return
    }
    setCharacter({ ...character, flaws: [...currentFlaws, { id: flaw.id, name: flaw.name, cost: flawCost }] })
  ...
  const saveCharacter = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast({ title: "Character Saved", description: "Your character has been updated" })
        router.push(`/characters/${characterId}`)
      } else {
        const error = await response.json()
        toast({ title: "Error", description: error.error || "Failed to save character", variant: "destructive" })
      }
    } catch (error) {
  ...
  if (status === "loading" || isLoading) {
    return (
      <div
        className="min-h-screen grimoire-bg flex items-center justify-center gap-3"
  ...
  return (
    <>
      {/* ══ Outer shell — matches app/page.tsx branded container ══ */}
      <div className="min-h-screen relative z-[1] py-5 px-3 md:py-7 md:px-4 grimoire-bg">
        ...
        >
          ...
          {/* Top hairline — gold spark → purple fade */}
          <div
          ...
          {/* ── Page header strip ── */}
          <div
            className="flex items-center justify-between px-5 sm:px-7 md:px-10 py-4 sm:py-5"
            style={{ borderBottom: "1px solid hsl(var(--border) / 0.5)" }}
          >
            {/* Back link — ghost chip matching header quick-links */}
            <button
              type="button"
            ...
            {/* Page title — centred */}
            <h1
              className="font-serif font-black uppercase text-primary hidden sm:block"
              style={{ fontSize: "clamp(0.85rem, 2vw, 1.1rem)", letterSpacing: "0.1em" }}
            >
              Edit:&nbsp;
              <span style={{ color: "hsl(var(--accent))" }}>{character.name}</span>
            </h1>

            {/* Save button — gold pill */}
            <button
              type="button"
              ...
              className="group relative overflow-hidden inline-flex items-center gap-2
                ...
                font-serif text-[10px] sm:text-[11px] uppercase tracking-[0.16em] font-bold
          ...
          {/* ── Content area — crosshatch background ── */}
          <div
          ...
          >
            ...
            <Tabs defaultValue="basic" className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <Label>Player</Label>
                        <Input value={character.player || ""} onChange={(e) => updateCharacter("player", e.target.value)} />
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <Input value={character.faction} onChange={(e) => updateCharacter("faction", e.target.value)} />
                      </div>
                      <div>
                        <Label>Nature</Label>
                        <Input value={character.nature || ""} onChange={(e) => updateCharacter("nature", e.target.value)} />
                      </div>
                      <div>
                        <Label>Demeanor</Label>
                        <Input value={character.demeanor || ""} onChange={(e) => updateCharacter("demeanor", e.target.value)} />
                      </div>
                      <div>
                        <Label>Essence</Label>
                        <Input value={character.essence || ""} onChange={(e) => updateCharacter("essence", e.target.value)} />
                      </div>
                      <div>
                        <Label>Sect</Label>
                        <Input value={character.sect || ""} onChange={(e) => updateCharacter("sect", e.target.value)} />
                      </div>
                      <div className="col-span-2">
                        <Label>Concept</Label>
                        <Textarea value={character.concept || ""} onChange={(e) => updateCharacter("concept", e.target.value)} />
                      </div>
                      <div className="col-span-2">
                        <Label>Avatar Description</Label>
                        <Textarea value={character.avatar || ""} onChange={(e) => updateCharacter("avatar", e.target.value)} />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4 pt-4">
                      <div>
                        ...
                        <div className="mt-2">
                          <DotSelector label="Arete" value={character.arete || 1} max={10} onChange={(v) => updateCharacter("arete", v)} />
                        </div>
                      </div>
                      <div>
                        <Label>Willpower ({character.willpower || 5})</Label>
                        <div className="mt-2">
                          <DotSelector label="Willpower" value={character.willpower || 5} max={10} onChange={(v) => updateCharacter("willpower", v)} />
                        </div>
                      </div>
                    </div>
                  </CardContent>
                    ...
                    <div className="grid grid-cols-3 gap-6">
                      <div>
                        ...
                        {character.attributes && (
                          <>
                            <DotSelector label="Strength"  value={character.attributes.strength  || 1} onChange={(v) => updateAttribute("strength",  v)} />
                            <DotSelector label="Dexterity" value={character.attributes.dexterity || 1} onChange={(v) => updateAttribute("dexterity", v)} />
                            <DotSelector label="Stamina"   value={character.attributes.stamina   || 1} onChange={(v) => updateAttribute("stamina",   v)} />
                          </>
                        )}
                      </div>
                      <div>
                        ...
                        {character.attributes && (
                          <>
                            <DotSelector label="Charisma"     value={character.attributes.charisma     || 1} onChange={(v) => updateAttribute("charisma",     v)} />
                            <DotSelector label="Manipulation" value={character.attributes.manipulation || 1} onChange={(v) => updateAttribute("manipulation", v)} />
                            <DotSelector label="Appearance"   value={character.attributes.appearance   || 1} onChange={(v) => updateAttribute("appearance",   v)} />
                          </>
                        )}
                      </div>
                      <div>
                        ...
                        {character.attributes && (
                          <>
                            <DotSelector label="Perception"   value={character.attributes.perception   || 1} onChange={(v) => updateAttribute("perception",   v)} />
                            <DotSelector label="Intelligence" value={character.attributes.intelligence || 1} onChange={(v) => updateAttribute("intelligence", v)} />
                            <DotSelector label="Wits"         value={character.attributes.wits         || 1} onChange={(v) => updateAttribute("wits",         v)} />
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-6 max-h-[600px] overflow-y-auto">
                      <div>
                        ...
                        {character.abilities && [
                          ...
                          <DotSelector key={ability} label={ability} value={character.abilities[ability] || 0} max={5} onChange={(v) => updateAbility(ability, v)} />
                        ))}
                      </div>
                      <div>
                        ...
                        {character.abilities && [
                          ...
                          <DotSelector key={ability} label={ability === 'martialArts' ? 'Martial Arts' : ability} value={character.abilities[ability] || 0} max={5} onChange={(v) => updateAbility(ability, v)} />
                        ))}
                      </div>
                      <div>
              ...
              {/* Spheres Tab — unchanged */}
              <TabsContent value="spheres" className="space-y-4">
                <Card>
                  <CardHeader><CardTitle>Spheres</CardTitle></CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-3 gap-4">
                      {character.spheres && Object.keys(character.spheres).map(sphere => (
                        <DotSelector key={sphere} label={sphere} value={character.spheres[sphere] || 0} max={5} onChange={(v) => updateSphere(sphere, v)} />
                      ))}
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    {character.backgrounds && Object.keys(character.backgrounds).map(bg => (
                      <DotSelector key={bg} label={bg.replace(/([A-Z])/g, ' $1').trim()} value={character.backgrounds[bg] || 0} max={5} onChange={(v) => updateBackground(bg, v)} />
                    ))}
                    ...
                    <CardHeader className="flex flex-row items-center justify-between">
                      ...
                      <Dialog open={meritsDialogOpen} onOpenChange={setMeritsDialogOpen}>
                        <DialogTrigger asChild>
                          <Button size="sm" className="gap-1"><Plus className="w-4 h-4" />Add Merit</Button>
                        </DialogTrigger>
                          ...
                          <ScrollArea className="h-[400px]">
                            <div className="space-y-2">
                              {meritsList.map(merit => (
                                <Card key={merit.id} className="p-3 cursor-pointer hover:bg-accent" onClick={() => addMerit(merit)}>
                        ...
                        <div className="space-y-2">
                          {character.merits.map((merit, idx) => (
                            <div key={idx} className="flex justify-between items-center p-2 bg-accent/10 rounded">
                              ...
                              <Button size="sm" variant="ghost" onClick={() => removeMerit(idx)}><X className="w-4 h-4" /></Button>
                            </div>
                          ))}
                        </div>
                      ) : (
                  ...
                  <Card>
                    <CardHeader className="flex flex-row items-center justify-between">
                      ...
                      <Dialog open={flawsDialogOpen} onOpenChange={setFlawsDialogOpen}>
                        <DialogTrigger asChild>
                          <Button size="sm" variant="secondary" className="gap-1"><Plus className="w-4 h-4" />Add Flaw</Button>
                        </DialogTrigger>
                        <DialogContent className="max-w-2xl max-h-[80vh]">
                          <DialogHeader>
                          ...
                          <ScrollArea className="h-[400px]">
                            <div className="space-y-2">
                              {flawsList.map(flaw => (
                                <Card key={flaw.id} className="p-3 cursor-pointer hover:bg-accent" onClick={() => addFlaw(flaw)}>
                                  <div className="flex justify-between items-start">
                                    <div>
                        ...
                        <div className="space-y-2">
                          {character.flaws.map((flaw, idx) => (
                            <div key={idx} className="flex justify-between items-center p-2 bg-green-50 rounded">
                              ...
                              <Button size="sm" variant="ghost" onClick={() => removeFlaw(idx)}><X className="w-4 h-4" /></Button>
                            </div>
                          ))}
                        </div>
                      ) : (
